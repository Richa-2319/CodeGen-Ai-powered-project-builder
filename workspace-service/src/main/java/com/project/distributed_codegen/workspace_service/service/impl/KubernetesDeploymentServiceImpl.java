...
import com.project.distributed_codegen.workspace_service.service.DeploymentService;
...
public class KubernetesDeploymentServiceImpl implements DeploymentService {
    ...
    @Value("${app.preview.enabled:false}")
    ...
    @Value("${app.preview.scheme:https}")
    ...
    public DeployResponse deploy(Long projectId) {
        if (!previewEnabled) {
            throw new ResponseStatusException(
                    ...
                    "Preview deployments are disabled until an isolated runner environment is configured");
        }
        ...
        boolean defaultPort = (previewScheme.equals("https") && proxyPort.equals("443"))
                || (previewScheme.equals("http") && proxyPort.equals("80"));
        String formattedUrl = previewScheme + "://" + domain + (defaultPort ? "" : ":" + proxyPort);
    ...
    private DeployResponse claimAndStartNewPod(Long projectId, String domain, String formattedUrl) {
        ...
        try {
            ...
            String startCmd = "npm install --ignore-scripts --no-audit --no-fund "
