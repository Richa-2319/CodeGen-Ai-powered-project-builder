...
public class PromptUtils {
    ...
    public final static String CODE_GENERATION_SYSTEM_PROMPT = """
            ...
            The app runs in an isolated browser preview. Use React, react-dom, react-router-dom,
            lucide-react, uuid (named exports such as v4), date-fns, zod, recharts,
            react-hook-form, @tanstack/react-query, clsx, tailwind-merge, or the provided
            @/components/ui components. Implement other helpers in project source instead of
            adding arbitrary npm packages. Use CSS for animation, not motion/framer-motion.
            External APIs, remote scripts/fonts/images, servers and package-install scripts
            are unavailable. Browser storage is in memory and resets when preview restarts.
            Use local data and inline SVG/CSS visuals. Tell the user when a requested feature
            needs a separate backend or persistent storage outside this preview environment.
