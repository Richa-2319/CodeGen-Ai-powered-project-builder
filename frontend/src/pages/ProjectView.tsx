    ...
    <div className="h-screen flex flex-col overflow-hidden bg-background">
      ...
      <header className="h-12 shrink-0 border-b border-border/50 bg-panel flex items-center justify-between px-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground text-xs ml-2">Build, preview and publish</span>
          {project?.role !== 'VIEWER' && (
            <DropdownMenu>
        ...
        <div className="flex items-center gap-2">
          ...
          {project?.role !== 'VIEWER' && (
            <>
              <PlanDialog />
              <PublishDialog projectId={projectId} disabled={project?.role !== "OWNER" || isStreaming} />
            </>
          )}
          <Button
            variant="ghost"
            ...
            <div className="h-full">
              <div className="h-full relative">
                <div className={cn("h-full absolute inset-0", viewMode !== "code" && "hidden")}>
                  <CodePanel key={projectId} projectId={projectId} updatedFiles={updatedFiles} readOnly={project?.role === "VIEWER" || isStreaming} onSaved={(path, content) => setUpdatedFiles(current => new Map(current).set(path, content))} />
                </div>
